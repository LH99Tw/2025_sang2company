...
[38](20251027):대시보드 히트맵 헤더 마스킹 적용
    ...
    - React ApexCharts 트리맵을 개편해 섹터 범례를 상단, 섹터 요약 칩을 하단에 배치하고, 각 종목 박스의 티커·수익률 텍스트 크기를 시총 비례로 조정했습니다. (`frontend/package.json`, `frontend/src/pages/Dashboard.tsx`)
        ...
        - 백테스트 페이지를 전면 재구성해 상단 입력 카드(그리드 배치) · 진행 현황 · 결과 섹션을 수직 배열로 정리하고, 컴팩트 입력 높이와 액션 버튼 행을 도입했습니다. (`frontend/src/pages/Backtest.tsx`, `frontend/src/components/common/GlassButton.tsx`)

[39](20251028):백테스트 전략 설정 헤더 슬림화 기획
    - 백테스트 페이지 상단 전략 설정 카드의 패딩과 컴포넌트 높이를 줄여 헤더처럼 얇게 보이도록 추가 슬림화 작업을 진행합니다.
    - Select/RangePicker/Input 공통 높이와 글자 크기를 재조정해 한 화면에 더 많은 필드를 두 줄로 배치할 수 있도록 준비합니다.
    - 액션 버튼과 보조 설명 텍스트의 여백을 맞춰 전체 카드 높이를 최소화하도록 후속 코드를 업데이트할 예정입니다.
    - 전략 설정 폼을 2행 구조로 재배치해 ① 알파 팩터/기간/리밸런싱 주기 ② 거래비용/분위수/액션 버튼 순으로 나열하고, 각 필드의 grid span을 조정합니다.
    - 페이지 상단 제목을 제거하고, 전략 설정 폼을 6열 그리드(3fr/3fr/1.5fr/1fr/1fr/2fr) 한 줄 구성으로 재정렬해 버튼까지 수평으로 정렬합니다.
    - 액션 버튼을 아이콘 전용으로 단순화하고, 입력 필드 레이블과 아이콘의 수직 정렬을 재조정해 전체 행이 균형 있게 보이도록 개선합니다.
    - 리밸런싱 주기 컬럼 폭을 재조정해 라벨이 두 줄로 내려가지 않도록 하고, 전체 행 정렬을 마무리합니다.
    - 레이블의 라인 높이와 정렬을 통일해 모든 항목이 동일한 베이스라인을 공유하도록 UI 재정비를 완료합니다.
    - 필드 컨테이너의 정렬을 상단 기준으로 변경해 리밸런싱 주기 라벨이 다른 항목과 같은 높이에서 시작되도록 보정합니다.
    - 포트폴리오 설정 패널도 동일한 가로형 컨트롤 그리드와 컴팩트 입력 스타일을 적용해 좌측 패널 높이를 대폭 축소합니다.
    - 포트폴리오 페이지 상단 제목/설명을 제거하고, 알파 선택 입력 폭을 단축하며, 포트폴리오 생성 버튼을 아이콘만 남긴 단일 가로 메뉴로 재정렬합니다.
    - 포트폴리오 안내 문구를 제거하고, 선별 기준 토글의 색상을 노란색 계열로 맞춰 테마 일관성을 유지합니다.
    - SEC EDGAR 스크립트에 특정 티커(APP, BRK-B, EME, HOOD, IBKR, PSKY)만 채우는 유틸리티와 티커 변형 매핑을 추가해 결측 재무 데이터를 보강합니다.
